...
function getInitialDesignFromParams(params: ReturnType<typeof useLocalSearchParams>): MarketplaceDesign | null {
  ...
  const sourceCollection =
    getParamAsString(params.collection) || PHYSICAL_DESIGNS_COLLECTION_CANDIDATES[0];
  ...
  useEffect(() => {
    ...
    const hydrateDesign = async () => {
      ...
      const candidates = preferredCollection
        ? [preferredCollection, ...PHYSICAL_DESIGNS_COLLECTION_CANDIDATES.filter((name) => name !== preferredCollection)]

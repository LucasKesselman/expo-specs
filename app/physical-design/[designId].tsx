...
function getInitialDesignFromParams(
  ...
  const sourceCollection =
    ...
    PHYSICAL_DESIGNS_COLLECTION_CANDIDATES[0];
...
export default function PhysicalDesignDetailScreen() {
  ...
  const initialDesign = useMemo(
    () => getInitialDesignFromParams(params),
    [params],
  );
  ...
  useEffect(() => {
    ...
    const hydrateDesign = async () => {
      ...
      const candidates = preferredCollection
        ? [
            preferredCollection,
            ...PHYSICAL_DESIGNS_COLLECTION_CANDIDATES.filter(
      ...
      try {
        for (const collectionName of candidates) {
          try {
            ...
            if (isMounted) {
              const mapped = mapFirestoreDocToMarketplaceDesign(
                snapshot,
                collectionName,
              );
              setDesign(mapped);
